...
namespace MySTL {
    ...
    public:
        ...
        vector(size_t capacity, T val) : _data(nullptr), _size(capacity), _capacity(capacity) {
            _data = new T[_capacity]; // 分配内存
            for (size_t i = 0; i < _size; ++i) {
                _data[i] = val; // 初始化所有元素为 val
            }
        }

        vector(size_t size) : _data(nullptr), _size(size), _capacity(size) {
            _data = new T[_capacity]; // 分配内存
            for (size_t i = 0; i < _size; ++i) {
                _data[i] = T(); // 初始化为默认值（对 int 是 0）
            }
        }
